"""Transformation CSV specifique Carrefour depuis les HTML bruts."""
...
def _extract_record_from_html(html: str, url_hint: str) -> dict:
    ...
    if not record["description"]:
        paragraphs = [normalize_text(p.get_text(" ", strip=True)) for p in soup.select("p")]
        paragraphs = [p for p in paragraphs if p and len(p) > 30]
        record["description"] = paragraphs[0] if paragraphs else None
...
def transform_csv(raw_dir: Path | None = None) -> Path:
    ...
    if df.empty:
        logger.warning("Carrefour: aucune donnée HTML à transformer")
        return PROCESSED_DIR / "carrefour_empty.csv"
    ...
    output_file = output_dir / f"carrefour_final_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv"
